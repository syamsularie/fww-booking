...
type PaymentPersister interface {
	...
	GetPaymentDetailByReservationID(reservatioID int) (model.PaymentDetail, error)
}
...
func (r *PaymentRepository) GetPaymentDetailByReservationID(reservatioID int) (model.PaymentDetail, error) {
	var paymentDetail model.PaymentDetail
	query := `
				SELECT r.passenger_id, r.flight_number, r.seat_number, r.price, p.payment_status, p.payment_method, p.payment_code
				FROM payments p 
			...
			`
	row := r.DB.QueryRow(query, reservatioID)
	err := row.Scan(&paymentDetail.PassengerID, &paymentDetail.FlightNumber, &paymentDetail.SeatNumber, &paymentDetail.Price, &paymentDetail.PaymentStatus, &paymentDetail.PaymentMethod, &paymentDetail.PaymentCode)
	if err != nil {
		if err == sql.ErrNoRows {
			return paymentDetail, nil
...
func (r *PaymentRepository) UpdatePaymentStatus(payment model.PaymentPayRequest, status bool) error {
	query := `UPDATE payments SET payment_status = ?, payment_method = ? WHERE payment_code = ?`
	_, err := r.DB.Exec(query, status, payment.PaymentMethod, payment.PaymentCode)
	if err != nil {
		return err
	}
